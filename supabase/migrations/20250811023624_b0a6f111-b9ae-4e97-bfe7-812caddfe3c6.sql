...
-- Consolidated migration (retry): avoid direct updates to cron.job
...
-- 3) Cron job schedule for daily backups at 02:00 UTC (idempotent, no direct table updates)
DO $$
BEGIN
  BEGIN
    PERFORM cron.schedule('daily-content-backup', '0 2 * * *', 'select public.take_daily_backup();');
  EXCEPTION WHEN duplicate_object THEN
    -- Job already exists; leave as-is to avoid permission errors
    NULL;
  WHEN others THEN
    -- Ignore if role lacks permissions to modify cron jobs
    NULL;
  END;
END $$;
