-- Fixed idempotent migration with proper EXECUTE quoting
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Public can read site_settings" ON public.site_settings FOR SELECT USING (true)';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Public can read media-projects" ON storage.objects FOR SELECT USING (bucket_id = ''media-projects'')';
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Editors/Admin can manage media-projects" ON storage.objects FOR ALL USING (bucket_id = ''media-projects'' AND (public.has_role(auth.uid(), ''admin'') OR public.has_role(auth.uid(), ''editor''))) WITH CHECK (bucket_id = ''media-projects'' AND (public.has_role(auth.uid(), ''admin'') OR public.has_role(auth.uid(), ''editor'')))';
  END IF;
END$$;